function currency(n){return new Intl.NumberFormat('vi-VN',{style:'currency',currency:'VND'}).format(n)}

function getCart(){return JSON.parse(localStorage.getItem('minishop_cart')||'{}')}
function saveCart(cart){localStorage.setItem('minishop_cart',JSON.stringify(cart))}

function addToCart(id){
	const product = (window.PRODUCTS||[]).find(p=>p.id==id); if(!product) return;
	const cart = getCart();
...
function renderDetail(id){
	...
	if(!p){ root.innerHTML = '<p>Sản phẩm không tồn tại.</p>'; return }
	root.innerHTML = `
	...
	document.getElementById('buyNow').addEventListener('click',()=>{ addToCart(id); window.location.href='../HTML/thanhtoan.html'; });
}
